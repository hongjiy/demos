...
char read_push_buttons(void) {
	...
	if((PINC & 0b00100000) == 0b00000000){ // Button 6 is being pressed
		return 6;
	}
	
	else if((PINC & 0b00010000) == 0b00000000){ // Button 5 is being pressed
		return 5;
	}
	
	else if((PINC & 0b00001000) == 0b00000000){ // Button 4 is being pressed
		return 4;
	}
	
	else if((PINC & 0b00000100) == 0b00000000){ // Button 3 is being pressed
		return 3;
	}
	
	else if((PINC & 0b00000010) == 0b00000000){ // Button 2 is being pressed
		return 2;
	}
	
	else if((PINC & 0b00000001) == 0b00000000){ // Button 1 is being pressed
		return 1;
	}
...
void  move_stepper_motor_by_step(int num_steps, int direction) {
	int i;
	if(direction > 0){ // Move motor clockwise
		for(i = 0; i < num_steps; i++){
			PORTE = (PORTE & 0b00001111) | 0b00010000;
			wait_ms(15);
			PORTE = (PORTE & 0b00001111) | 0b00100000;
			wait_ms(15);
			PORTE = (PORTE & 0b00001111) | 0b01000000;
			wait_ms(15);
			PORTE = (PORTE & 0b00001111) | 0b10000000;
			wait_ms(15);
			PORTE = (PORTE & 0b00001111) | 0b00010000;
			wait_ms(15);
		}
	}
	else if(direction < 0){ // Move motor counterclockwise
		for(i = 0; i < num_steps; i++){
			PORTE = (PORTE & 0b00001111) | 0b10000000;
			wait_ms(15);
			PORTE = (PORTE & 0b00001111) | 0b01000000;
	...
	else{} //Motor does nothing when direction == 0
	
	PORTE = PORTE & 0b0001111;
}